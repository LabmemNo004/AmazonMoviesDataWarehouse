...
def connect_to_web(URL,order): # 访问并处理数据
    ...
    try: # 判定网站类型
        ...
        try:  # 获取Movie&TV名称
            ...
            try:
                dog=driver.find_element_by_css_selector("[alt='Dogs of Amazon']").text
            except:
                time.sleep(3)
                print("\nWebsite {} kill the robot ，jump！".format(order)) # 验证码异常处理
                resetUrl(URL)
            else:
                time.sleep(3)
                print("\nWebsite {} kill the dog ，jump！".format(order))  # 无法访问页面异常处理
            try:
                closeBrowser(driver)
            except:
                return
...
CSV='webUnsolved2.csv' # 存储data用CSV文件名
#UnSOLVED='unsolvedUrl42000.txt' # 存储访问失败url用txt文件名
Rest="unsolvedUrl12000.txt" # 待访问url文件名
Log="logUnsolved2.txt" # 记录指针
